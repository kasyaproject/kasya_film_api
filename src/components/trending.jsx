...
const trending = () => {
  ...
  useEffect(() => {
    getMovieTrending().then((results) => {
      setPopularMovieList(results);
    });
  }, []);
  // console.log({ ListPopular: PopularMovieList });

  const PopularMovie = () => {
    // Ambil hanya 6 data pertama dari PopularMovieList
    const limitedMovies = PopularMovieList.slice(0, 6);

    return limitedMovies.map((movie) => {
      return (
        <div
          key={movie.id}
          className="transition-transform duration-300 transform bg-yellow-50 hover:scale-105"
        >
          {/* IMage Film */}
          <div
            className="relative border-2 shadow-lg h-60 sm:h-80 group"
            style={{
              backgroundImage: `url(${import.meta.env.VITE_BASEIMGURL}${
                movie.backdrop_path
              })`,
